...
export default function Registro(): JSX.Element {
  ...
  const [formData, setFormData] = useState<RegistroCredenciales>({
    email: '',
    password: '',
    nombre: '',
  });
  ...
  const validarFormulario = (): boolean => {
    ...
    if (!formData.password) {
      ...
      nuevosErrores.password = 'La contraseña debe tener mayúsculas y minúsculas';
    }
  ...
  const handleSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      const mensaje = error instanceof Error ? error.message : 'Error al registrarse';
      toast.error(mensaje);
    }
  };
  ...
  const handleGoogleLogin = async (): Promise<void> => {
    try {
      ...
      toast.success('¡Cuenta creada exitosamente con Google! Bienvenido');
      navigate('/');
    } catch (error) {
      const mensaje = error instanceof Error ? error.message : 'Error al registrarse con Google';
      toast.error(mensaje);
    }
  };
        ...
        <div className="bg-card p-8 rounded-lg shadow-xl border border-border">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              />
              {errores.nombre && (
                <p className="mt-1 text-sm text-destructive">{errores.nombre}</p>
              )}
            </div>
            ...
            <div>
              ...
              {errores.confirmPassword && (
                <p className="mt-1 text-sm text-destructive">{errores.confirmPassword}</p>
              )}
            </div>
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-2 bg-card text-muted-foreground">O registrarse con</span>
            </div>
          </div>
